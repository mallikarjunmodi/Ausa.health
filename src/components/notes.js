...
export default function Notes() {
  const [isChecked1, setIsChecked1] = useState(false);
    ...
    return (
        ...
        <input id='click3' type="checkbox"  class="absolute opacity-0 peer" onChange={(e) => setIsChecked1(e.target.checked)}/>
        <label for='click3' className="cursor-pointer w-full flex tracking-[1px] flex-col justify-start peer-checked:mb-2 mb-2 ">
          ...
          <div className={`absolute top-6 right-12 transform transition-transform duration-300 ease-in-out ${isChecked1 ? 'rotate-0' : 'rotate-180'}`}>
